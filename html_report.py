...
"""
    ...
    for query_name, anomalies in anomalies_by_query.items():
        ...
        severe_count = sum(1 for a in anomalies if a.get('yoy_pct', 0) < -95)
        moderate_count = sum(1 for a in anomalies if -95 <= a.get('yoy_pct', 0) < -85)
...
"""
            ...
            for item in items:
                # Determine severity class based on YoY percentage
                yoy_pct = item.get('yoy_pct', 0)
                if yoy_pct < -95:
                ...
                prior_year_count = item.get('prior_year_count', 0)
                yoy_change = item.get('yoy_change', 0)

                html += f"""                            <tr>
                                <td>{item['date_str']}</td>
                                <td>{item['day_name']}</td>
                                <td class="right">{item['count']:,}</td>
                                <td class="right">{prior_year_count:,}</td>
                                <td class="right">{yoy_change:+,}</td>
                                <td class="right {severity_class}">{yoy_pct:+.1f}%</td>
                            </tr>
"""
...
"""
    ...
    html += f"""        </div>
        ...
        <div class="footer">
            <p>Generated by Data Anomaly Detector</p>
            <p>Year-over-year comparison: flagging decreases >= 50% or counts below minimum threshold</p>
        </div>
    </div>
</body>
</html>
